...
import { createCountryDataAPI, updateCountryDataAPI, fetchCountryDataAPI } from '@/apis';
...
const CountryForm = () => {
    ...
    useEffect(() => {
        if (isEditMode && id) {
            const fetchCountry = async () => {
                try {
                    const response = await fetchCountryDataAPI(id);
                    if (response && response.result) {
                        setCountryName(response.result.name || '');
                    }
                } catch (err) {
                    setError('Không thể tải dữ liệu quốc gia: ' + err.message);
                }
            };
            fetchCountry();
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError('Lỗi khi lưu quốc gia: ' + (err.response?.message || err.message));
    ...
    return (
        <div>
            <div className={cx('page-header')}>
                ...
                <Link to={adminRouteConfig.listCountries} className={cx('button-outline')}>
                    <ArrowLeft size={16} />
                    Quay lại
                </Link>
            </div>
            ...
            {submitSuccess && (
                <div className={cx('alert', 'alert-success')} style={{ marginBottom: '1rem' }}>
                    {isEditMode ? 'Cập nhật quốc gia thành công!' : 'Tạo quốc gia mới thành công!'}
                </div>
            )}
