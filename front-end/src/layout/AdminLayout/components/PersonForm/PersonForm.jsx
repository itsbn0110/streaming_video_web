...
import { createPersonDataAPI, updatePersonDataAPI, fetchPersonDataAPI } from '@/apis';
...
const PersonForm = ({ editMode = false, personData = null, personType }) => {
    ...
    // Thêm useEffect để fetch dữ liệu person khi editMode
    useEffect(() => {
        if (editMode && id) {
            const fetchPerson = async () => {
                try {
                    const response = await fetchPersonDataAPI(id);
                    if (response && response.result) {
                        ...
                        setFormData({
                            ...
                            role: person.role || (personType === 'actor' ? 'Actor' : 'Director'),
                            personAvatar: person.avatar || null,
                            personAvatarPreview: null,
                        });
                    }
                } catch (err) {
                    setError('Không thể tải dữ liệu person: ' + err.message);
                }
            };
            fetchPerson();
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response && response.result) {
                alert(
                    editMode
                        ? `${personType === 'actor' ? 'Diễn viên' : 'Đạo diễn'} đã được cập nhật!`
                        : `${personType === 'actor' ? 'Diễn viên' : 'Đạo diễn'} đã được tạo thành công!`,
                );
                navigate(personType === 'actor' ? adminRouteConfig.listActors : adminRouteConfig.listDirectors);
            } else {
                throw new Error(response.message || 'Có lỗi xảy ra');
            }
        } catch (err) {
            setError(err.message || `Có lỗi xảy ra khi lưu ${personType === 'actor' ? 'diễn viên' : 'đạo diễn'}`);
            ...
            <div className={cx('page-header')}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                    <button
                        onClick={() =>
                            navigate(
                                personType === 'actor' ? adminRouteConfig.listActors : adminRouteConfig.listDirectors,
                            )
                        }
                        style={{
                            background: 'none',
            ...
            <div className={cx('card')}>
                <form className={cx('form-container')} onSubmit={handleSubmit}>
                    ...
                    <div className={cx('form-group')}>
                        ...
                        {(formData.personAvatarPreview || (editMode && formData.personAvatar)) && (
                            <img
